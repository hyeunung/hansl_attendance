...
SET annual_leave_granted_current_year = (
  CASE 
    ...
    WHEN EXTRACT(YEAR FROM join_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN
      LEAST(11, 12 - EXTRACT(MONTH FROM join_date) + 1)
    -- 2년차 이상: 15 + ((근속년수-2)/2의 정수부분)
    ELSE
      15 + GREATEST(0, (EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM join_date) + 1 - 2) / 2)::INTEGER
  END
)
WHERE join_date IS NOT NULL;

-- 3. join_date가 없는 직원은 기본값 15로 설정
UPDATE employees 
