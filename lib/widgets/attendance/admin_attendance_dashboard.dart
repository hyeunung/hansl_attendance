...
class AdminAttendanceDashboard extends StatefulWidget {
  ...
  @override
  State<AdminAttendanceDashboard> createState() =>
...
class _AdminAttendanceDashboardState extends State<AdminAttendanceDashboard> {
  ...
  Future<void> _loadAttendanceSummary() async {
    try {
      ...
      */
      ...
      try {
        ...
        for (var leave in allLeaves) {
          ...
          if (startDate.isBefore(todayDate.add(Duration(days: 1))) &&
            ...
            if (kDebugMode) {
              print(
                '  - 오늘 휴가/출장: user_email=${leave['user_email']}, type=${leave['type']}',
              );
            }
          }
        }
      ...
      final leaveEmployeeEmails = (onLeave)
          .map((l) => l['user_email']?.toString().trim().toLowerCase())
          .where((email) => email != null)
          .cast<String>()
          .toList();
      final businessTripEmployeeEmails = (onBusinessTrip)
          .map((l) => l['user_email']?.toString().trim().toLowerCase())
          .where((email) => email != null)
          .cast<String>()
          .toList();
      ...
      final leaveNames = (onLeave)
          .map((l) => l['name']?.toString().trim().toLowerCase())
          .where((name) => name != null && name.isNotEmpty)
          .cast<String>()
          .toList();
      final businessTripNames = (onBusinessTrip)
          .map((l) => l['name']?.toString().trim().toLowerCase())
          .where((name) => name != null && name.isNotEmpty)
          .cast<String>()
          .toList();
      final allAbsentNames = <String>{...leaveNames, ...businessTripNames};
      ...
      for (var record in attendanceRecords) {
        ...
        if (kDebugMode) {
          print(
            '🔍 출근 기록 처리: user_email=${record['user_email']}, clock_in=${record['clock_in']}',
          );
        }
        ...
        if (checkInTime.hour > 8 ||
            (checkInTime.hour == 8 && checkInTime.minute > 30)) {
          lateList.add(attendanceInfo);
          if (kDebugMode) {
            print(
              '  ➡️ 지각 처리: ${attendanceInfo['name']} (${checkInTime.hour}:${checkInTime.minute})',
            );
          }
        } else {
          ...
          if (kDebugMode) {
            print(
              '  ➡️ 정상 출근: ${attendanceInfo['name']} (${checkInTime.hour}:${checkInTime.minute})',
            );
          }
        }
      }
      ...
      final List<dynamic> effectiveRecords = allTodayRecords.where((rec) {
        ...
        final normalizedRecEmail = recEmail?.trim().toLowerCase();
        final recName = rec['employee_name']?.toString();
        final normalizedRecName = recName?.trim().toLowerCase();
        final bool isOnLeave =
            (normalizedRecEmail != null &&
                allAbsentEmployeeEmails.contains(normalizedRecEmail)) ||
            (normalizedRecName != null &&
                allAbsentNames.contains(normalizedRecName));
        return !isOnLeave;
      }).toList();
      ...
      for (var rec in effectiveRecords) {
        ...
        final bool isAbsentByStatus =
            recStatus == null || recStatus.isEmpty || recStatus == '미출근';
      ...
      if (kDebugMode) {
        print('\n========== 출근 현황 요약 ==========');
        print('📊 전체 직원 수: ${(allTodayRecords).length}명');
        print('✅ 정상 출근: ${normalList.length}명');
        for (var person in normalList) {
          print(
            '   - ${person['name']} (${person['department']}): ${person['check_in_time']}',
          );
        }
        ...
        print(
          '🏖️ 휴가/출장: ${leaveEmployeeEmails.length + businessTripEmployeeEmails.length}명',
        );
      ...
      final annualAndBizTripCount =
          (onLeave)
              .where((l) => l['type'] != 'half_am' && l['type'] != 'half_pm')
              .length +
          (onBusinessTrip).length;
      final actualLeaveCount =
          annualAndBizTripCount + halfAmNotAttendedCount + halfPmCount;

      setState(() {
        _attendanceSummary = {
          // 전체는 오늘자 레코드 전체 인원
          'total': (allTodayRecords).length,
        ...
        _absentEmployees = absentEmployeesList
  ...
  Widget build(BuildContext context) {
    ...
    final isManager =
        attendanceRole != null &&
        attendanceRole.toString().contains('_manager');
