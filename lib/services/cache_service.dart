...
class CacheService with TimerManagementMixin {
  ...
  Future<void> invalidatePattern(String pattern) async {
    ...
    final persistentKeys = allKeys
        .where((k) => k.startsWith(_persistentPrefix) && k.contains(pattern))
        .toList();
  ...
  Map<String, dynamic> getCacheStats() {
    ...
    final total = stats['memory_total'] as int;
    final valid = stats['memory_valid'] as int;

    return {
      'hit_rate': total > 0 ? valid / total : 0.0,
      'total_requests': _requests,
      'cache_hits': _hits,
      'cache_misses': _requests - _hits,
