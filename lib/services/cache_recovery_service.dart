...
class CacheRecoveryService {
  ...
  static Future<void> recoverLeaveDataFromCache() async {
    ...
    try {
      ...
      // 2. all_leaves 캐시 확인
      final allLeavesCache = await _cache.getOrFetch<List<Map<String, dynamic>>>(
        key: 'all_leaves',
        fallback: () async => <Map<String, dynamic>>[],
      );

      if (allLeavesCache != null && allLeavesCache.isNotEmpty) {
        AppLogger.debug('캐시 복구', 'all_leaves 캐시에서 ${allLeavesCache.length}개 발견');
      ...
      if (currentUser?.email != null) {
        ...
        final myLeavesCache = await _cache.getOrFetch<List<Map<String, dynamic>>>(
          key: myLeavesKey,
          fallback: () async => <Map<String, dynamic>>[],
        );

        if (myLeavesCache != null && myLeavesCache.isNotEmpty) {
          AppLogger.debug('캐시 복구', '$myLeavesKey 캐시에서 ${myLeavesCache.length}개 발견');
  ...
  static Future<void> _recoverSingleLeave(Map<String, dynamic> leaveData) async {
    try {
      // DB에 해당 ID가 있는지 확인
      ...
      final result = await _client.from('leave').insert(leaveToInsert).select().single();

      if (kDebugMode)
        print('✅ 복원됨: ${leaveData['type']} (${leaveData['user_email']}) -> DB ID: ${result['id']}');
    } catch (e) {
