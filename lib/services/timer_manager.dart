...
class TimerManager {
  ...
  int cancelAllTimers() {
    ...
    if (count > 0 && kDebugMode) {
      print('❌ Cancelled all timers: $count timers');
    }
  ...
  List<String> pauseAllTimers() {
    ...
    if (pausedKeys.isNotEmpty && kDebugMode) {
      print('⏸️ Paused ${pausedKeys.length} timers');
    }
  ...
  void _performMaintenance() {
    ...
    if (kDebugMode) {
      print('🧹 Timer maintenance: ${stats.activeTimers} active, '
  ...
  void dispose() {
    cancelAllTimers();
    if (kDebugMode) print('🗑️ TimerManager disposed');
  }
}
